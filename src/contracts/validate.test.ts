...
const newsFirstClone=():EpisodeManifest=>{
  const doc=clone();
  doc.formatProfile="news-first";
  doc.experienceVersion="news-first-v3.0";
  doc.scenes=doc.scenes.filter(scene=>scene.role!=="retrieval");
  return doc;
};

describe("EpisodeManifest v2.1",()=>{
  it("accepts the legacy v2.1 fixture",()=>{expect(validateEpisode(clone()).ok).toBe(true);});
  it("accepts news-first when the story is uninterrupted and replay is after it",()=>{expect(validateEpisode(newsFirstClone()).ok).toBe(true);});
  it("requires an explicit experience version for production news-first",()=>{const doc=newsFirstClone();doc.kind="production";delete doc.experienceVersion;const result=validateEpisode(doc);expect(result.ok).toBe(false);if(!result.ok)expect(result.issues.some(i=>i.code==="E_SCHEMA"&&i.detail.includes("experienceVersion"))).toBe(true);});
  it("rejects an unknown experience version",()=>{const doc=newsFirstClone();(doc as unknown as {experienceVersion:string}).experienceVersion="news-first-v99.0";const result=validateEpisode(doc);expect(result.ok).toBe(false);if(!result.ok)expect(result.issues.some(i=>i.code==="E_SCHEMA")).toBe(true);});
  ...
  it("does not allow retrieval in news-first",()=>{const doc=clone();doc.formatProfile="news-first";doc.experienceVersion="news-first-v3.0";const result=validateEpisode(doc);expect(result.ok).toBe(false);if(!result.ok)expect(result.issues.some(i=>i.code==="E_STRUCTURE"&&i.detail.includes("retrieval"))).toBe(true);});
  it("requires explicit learning blocks to stay after the final story scene",()=>{const doc=newsFirstClone();const phrase=doc.scenes.find(scene=>scene.role==="phrase");if(!phrase)throw new Error("fixture phrase missing");const withoutPhrase=doc.scenes.filter(scene=>scene!==phrase);doc.scenes=[...withoutPhrase.slice(0,2),phrase,...withoutPhrase.slice(2)];const result=validateEpisode(doc);expect(result.ok).toBe(false);if(!result.ok)expect(result.issues.some(i=>i.code==="E_STRUCTURE"&&i.detail.includes("after the final story scene"))).toBe(true);});
});
