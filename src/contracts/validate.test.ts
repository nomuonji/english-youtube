...
describe("EpisodeManifest v2.1",()=>{
  ...
  it("rejects an unknown experience version",()=>{const doc=newsFirstClone() as EpisodeManifest&{experienceVersion:string};doc.experienceVersion="news-first-v99.0";const result=validateEpisode(doc);expect(result.ok).toBe(false);if(!result.ok)expect(result.issues.some(i=>i.code==="E_SCHEMA")).toBe(true);});
