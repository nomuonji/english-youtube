...
import {reviewRetention} from "../editorial/retention";
...
try{
  ...
  if(!validated.ok){
    ...
    const review=reviewRetention(validated.value);
    console.log(JSON.stringify({ok:review.hardFailures.length===0&&review.score>=80,path,...review},null,2));
    if(review.hardFailures.length>0||review.score<80)process.exitCode=3;
  }
}catch(error){
