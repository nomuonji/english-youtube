...
export const EpisodeVideo:React.FC<EpisodeVideoProps>=({manifest,resolved,imageAssets=[],brollAssets=[]})=>{
  ...
  const modern=newsFirstV3||documentaryV4;
  return <AbsoluteFill style={{backgroundColor:modern?"#050A10":"#F4F1E8"}}>
    <Html5Audio src={staticFile("generated/bgm.wav")} volume={documentaryV4?0.12:newsFirstV3?0.14:0.105}/>
    ...
    {resolved.scenes.flatMap(scene=>{
      ...
      if((scene.chapterCallFrames??0)>0)events.push(<Sequence key={`sfx-chapter-${scene.sceneId}`} from={scene.startFrame} durationInFrames={Math.min(scene.chapterCallFrames??90,90)}><Html5Audio src={staticFile("generated/sfx/chapter-whoosh.wav")} volume={documentaryV4?0.20:newsFirstV3?0.26:0.34}/></Sequence>);
      if(manifestScene.role==="hook")events.push(<Sequence key={`sfx-hook-${scene.sceneId}`} from={scene.startFrame+4} durationInFrames={30}><Html5Audio src={staticFile("generated/sfx/hook-impact.wav")} volume={documentaryV4?0.25:0.30}/></Sequence>);
      if(manifestScene.visual.type==="metric")events.push(<Sequence key={`sfx-metric-${scene.sceneId}`} from={scene.startFrame+12} durationInFrames={24}><Html5Audio src={staticFile("generated/sfx/metric-hit.wav")} volume={documentaryV4?0.20:0.24}/></Sequence>);
      if(manifestScene.role==="phrase")events.push(<Sequence key={`sfx-phrase-${scene.sceneId}`} from={scene.startFrame} durationInFrames={20}><Html5Audio src={staticFile("generated/sfx/phrase-ping.wav")} volume={documentaryV4?0.18:0.24}/></Sequence>);
