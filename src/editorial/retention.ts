...
export type RetentionReview={score:number;hardFailures:RetentionIssue[];warnings:RetentionIssue[];metrics:{hookQuestion:boolean;forwardPullScenes:number;concreteStoryScenes:number;maxVisualRun:number;learningAnchorSpread:number;storyScenes:number}};

const GENERIC_INTRO=/^(today\b|in this video\b|welcome\b|let'?s (talk|look|learn|discuss)\b|this video (is|will)\b)/i;
const FORWARD_PULL=/\b(but|however|yet|the catch|the problem|the question|why|what happens|which means|this is where|next|surprise|turns out|not so simple|here is the twist)\b/i;
...
const isConcrete=(m:EpisodeManifest,s:Scene):boolean=>{
  if(["metric","chain","compare","timeline"].includes(s.visual.type))return true;
  return NUMBER.test(sceneText(m,s));
};
...
export const reviewRetention=(m:EpisodeManifest):RetentionReview=>{
  ...
  if(!hook)hard("R_HOOK","scenes","hook scene is missing");
    ...
    const wc=hookText.match(/[A-Za-z0-9]+(?:['’-][A-Za-z0-9]+)*/g)?.length??0;
    if(wc>65)warn("R_HOOK_LONG",hook.id,`hook is ${wc} words; target is a fast 15–25 second opening`);
  }
  ...
  if(learningAnchorSpread<.3)warn("R_LEARNING_SPREAD","learningPoints","learning anchors are clustered; distribute useful-English discoveries across the episode");
  ...
  return {score,hardFailures:issues.filter(i=>i.severity==="hard"),warnings:issues.filter(i=>i.severity==="warn"),metrics:{hookQuestion,forwardPullScenes:forward,concreteStoryScenes:concrete,maxVisualRun:visualRun,learningAnchorSpread:Number(learningAnchorSpread.toFixed(3)),storyScenes:stories.length}};
};
