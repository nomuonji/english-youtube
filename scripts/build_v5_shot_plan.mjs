...
const queryFor=text=>{
  ...
  if(/chip|gpu|semiconductor/.test(t))return "semiconductor GPU computer chip close up";
  if(/data cent|server|cloud|computing/.test(t))return "data center server racks computing";
  if(/substation/.test(t))return "electric substation high voltage";
  if(/transmission|grid|power line/.test(t))return "high voltage electricity transmission power lines";
  if(/power generation|power plant|electricity/.test(t))return "electric power generation infrastructure";
  if(/ipo|share|invest|capital|financ/.test(t))return "stock market investment finance technology";
  if(/construction|permit|build/.test(t))return "electric infrastructure construction";
  return "technology infrastructure electricity";
};
const cameraFor=id=>{
...
const cutPointsFor=rs=>{
  const MIN=92;const MAX=210;const points=[0];
  const cueStarts=uniq(rs.cues.map(c=>c.startFrame)).sort((a,b)=>a-b);
  for(const start of cueStarts){
    const last=points.at(-1);
    if(start-last>=MIN&&rs.durationFrames-start>=45)points.push(start);
  }
  let cursor=points.at(-1);
  while(rs.durationFrames-cursor>MAX){cursor+=180;points.push(cursor);}
  if(points.at(-1)!==rs.durationFrames)points.push(rs.durationFrames);
  return points;
};
...
for(const rs of resolved.scenes){
  ...
  for(let i=0;i<segments.length;i++){
    ...
    shots.push({
      ...
      sourceLabel,
      metric,
      searchQuery:queryFor(`${headline} ${text}`),
...
console.log(JSON.stringify({ok:true,shots:shots.length,first36Shots:first36.length,first36Kinds:first36.map(s=>s.kind),plan:planArg,props:outPropsArg}));
