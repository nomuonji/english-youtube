...
const juicyWide = files.find(path => path.endsWith("animation-english-juicy-wide-540p.mp4"));
const juicyShort = files.find(path => path.endsWith("animation-english-juicy-short-540p.mp4"));
const juicyWideSheet = files.find(path => path.endsWith("animation-english-juicy-wide-contact-sheet.jpg"));
const juicyShortSheet = files.find(path => path.endsWith("animation-english-juicy-short-contact-sheet.jpg"));
...
if ([juicyWide, juicyShort, juicyWideSheet, juicyShortSheet].some(Boolean) && ![juicyWide, juicyShort, juicyWideSheet, juicyShortSheet].every(Boolean)) throw new Error("Juicy review artifact is incomplete");
if (juicyWide) {
  copyFileSync(juicyWide, join(outputDir, "juicy-wide.mp4"));
  copyFileSync(juicyShort, join(outputDir, "juicy-short.mp4"));
  copyFileSync(juicyWideSheet, join(outputDir, "juicy-wide-contact-sheet.jpg"));
  copyFileSync(juicyShortSheet, join(outputDir, "juicy-short-contact-sheet.jpg"));
}
const juicyBlock = juicyWide ? `<h2>全編の演出強化版（候補C）</h2><p class="note">配達とソフトウェア公開の物語全編で、障害・迂回・別場面・発話・想起の各イベントに、動き、粒子、カメラ、音の演出を割り当てました。同じ台本と実測音声を使っています。</p><div class="variant-grid"><div><h3>横型 16:9</h3><div class="video"><video controls playsinline preload="metadata" src="./juicy-wide.mp4"></video></div><img src="./juicy-wide-contact-sheet.jpg" alt="Juicy wide contact sheet"></div><div><h3>縦型 9:16</h3><div class="video"><video class="vertical-video" controls playsinline preload="metadata" src="./juicy-short.mp4"></video></div><img src="./juicy-short-contact-sheet.jpg" alt="Juicy short contact sheet"></div></div>` : "";
...
*{box-sizing:border-box}body{margin:0;background:#081722;color:#fff6e8;font-family:Inter,"Noto Sans JP",system-ui,sans-serif}main{width:min(1180px,calc(100% - 24px));margin:auto;padding:28px 0 70px}.eyebrow{color:#69dceb;font-size:12px;font-weight:900;letter-spacing:.16em}.title{font-size:clamp(28px,5vw,56px);line-height:1.06;margin:14px 0}.meta{display:flex;gap:9px;flex-wrap:wrap;margin-bottom:22px}.chip{border:1px solid #3f6a78;background:#152f3e;border-radius:99px;padding:7px 12px;font-size:13px}.video{border:1px solid #345566;border-radius:16px;overflow:hidden;background:#000}.video video{display:block;width:100%;aspect-ratio:16/9}.video video.vertical-video{aspect-ratio:9/16;max-height:650px;margin:auto;width:auto;max-width:100%}.controls{display:flex;gap:8px;padding:12px;background:#102636}.controls button{background:#1e4253;color:#fff6e8;border:1px solid #538092;border-radius:8px;padding:8px 13px;cursor:pointer}h2{margin-top:34px;font-size:20px}h3{font-size:16px}.variant-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:20px}.variant-grid img{margin-top:12px}img{width:100%;border-radius:10px}.note{color:#b3c5c8;line-height:1.7;font-size:14px}@media(max-width:760px){.variant-grid{grid-template-columns:1fr}}
</style></head><body><main><div class="eyebrow">WORLD IN CLEAR ENGLISH / CANDIDATE REVIEW</div><h1 class="title">${esc(lesson.title)}</h1><div class="meta"><span class="chip">表現: ${esc(lesson.target.phrase)}</span><span class="chip">${esc(lesson.level)}</span><span class="chip">${(durationFrames/fps).toFixed(1)}秒</span></div>${juicyBlock}<h2>初期案（候補A）</h2><div class="video"><video id="preview" controls playsinline preload="metadata" src="./video.mp4"></video><div class="controls"><button data-rate="0.75">0.75×</button><button data-rate="1">1×</button><button data-rate="1.25">1.25×</button><button id="restart">↺ 最初から</button></div></div><h2>候補Aの画面の流れ</h2><img src="./contact-sheet.jpg" alt="Animation English contact sheet">${kineticBlock}<p class="note">英語学習フォーマットの試作です。このページでの確認は YouTube 公開承認ではありません。場面で理解できるか、声に出す時間が足りるか、最後に表現を思い出せるかを確認してください。</p></main><script>const video=document.getElementById('preview');document.querySelectorAll('[data-rate]').forEach(button=>button.addEventListener('click',()=>{video.playbackRate=Number(button.dataset.rate);video.play()}));document.getElementById('restart').addEventListener('click',()=>{video.currentTime=0;video.play()});</script></body></html>`;
